...
namespace ElasticScaleStarterKit
{
    internal class Program
    {
        ...
        private static void PrintMenu()
        {
            ...
            ConsoleUtils.WriteColor(createSmmColor, "1. Range:Create shard map manager, and add a couple shards");
            ConsoleUtils.WriteColor(otherMenuItemColor, "2. Range:Add another shard");
            ConsoleUtils.WriteColor(otherMenuItemColor, "3. Range:Insert sample rows using Data-Dependent Routing");
            ConsoleUtils.WriteColor(otherMenuItemColor, "4. Range:Execute sample Multi-Shard Query");
            ConsoleUtils.WriteColor(otherMenuItemColor, "5. Range:Drop shard map manager database and all shards");

            ConsoleUtils.WriteColor(createSmmColor, "6. List:Create shard map manager, and add a couple shards");
            ConsoleUtils.WriteColor(otherMenuItemColor, "7. List:Add another shard");
            ConsoleUtils.WriteColor(otherMenuItemColor, "8. List:Insert sample rows using Data-Dependent Routing");
            ConsoleUtils.WriteColor(otherMenuItemColor, "9. List:Execute sample Multi-Shard Query");
            ConsoleUtils.WriteColor(otherMenuItemColor, "10. List:Drop shard map manager database and all shards");
            ConsoleUtils.WriteColor(EnabledColor, "11. Exit");
        }
        ...
        private static bool GetMenuChoiceAndExecute()
        {
            while (true)
            {
                ...
                switch (inputValue)
                {
                    ...
                    // Add for list shard
                    case 6: // Create shard map manager
                        Console.WriteLine();
                        CreateShardMapManagerAndShard2();
                        return true;
                    //case 7: // Add shard
                    //    Console.WriteLine();
                    //    AddShard2();
                    //    return true;
                    case 8: // Data Dependent Routing
                        Console.WriteLine();
                        DataDepdendentRouting2();
                        return true;
                    case 9: // Multi-Shard Query
                        Console.WriteLine();
                        MultiShardQuery2();
                        return true;
                    case 10: // Drop all
        ...
        private static void CreateShardMapManagerAndShard2()
        {
            if (s_shardMapManager != null)
            {
                ConsoleUtils.WriteWarning("Shard Map Manager already exists");
            ...
            if (!shardMap.GetShards().Any())
            {
                CreateShardSample.CreateShard2(shardMap, 0);
                CreateShardSample.CreateShard2(shardMap, 1);
                CreateShardSample.CreateShard2(shardMap, 2);
                CreateShardSample.CreateShard2(shardMap, 3);
                CreateShardSample.CreateShard2(shardMap, 4);
                CreateShardSample.CreateShard2(shardMap, 5);
                CreateShardSample.CreateShard2(shardMap, 6);
                CreateShardSample.CreateShard2(shardMap, 7);
                CreateShardSample.CreateShard2(shardMap, 8);
                CreateShardSample.CreateShard2(shardMap, 9);
            }
        }
        ...
        //private static void AddShard2()
        //{
        //    List<int> shardMap = TryGetShardMap2();
        //    if (shardMap != null)
        //    {
        //        // Here we assume that the ranges start at 0, are contiguous, 
        //        // and are bounded (i.e. there is no range where HighIsMax == true)
        ...
        private static void DataDepdendentRouting2()
        {
            for (int i = 0; i < 9; i++)
            {
                ListShardMap<int> shardMap = TryGetShardMap2();
                if (shardMap != null)
                {
                    DataDependentRoutingSample.ExecuteDataDependentRoutingQuery(
                        shardMap,
                        Configuration.GetCredentialsConnectionString(),i);
                }
            }
        ...
        private static void MultiShardQuery2()
        {
            ListShardMap<int> shardMap = TryGetShardMap2();
            if (shardMap != null)
            {
        ...
        private static ListShardMap<int> TryGetShardMap2()
        {
            if (s_shardMapManager == null)
            {
                ConsoleUtils.WriteWarning("Shard Map Manager has not yet been created");
