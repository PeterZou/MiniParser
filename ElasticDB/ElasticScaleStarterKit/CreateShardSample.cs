...
namespace ElasticScaleStarterKit
{
    internal class CreateShardSample
    {
        ...
        public static void CreateShard2(ListShardMap<int> shardMap, int rangeForNewShard)
        {
            Shard shard = default(Shard);
            // Create a new shard, or get an existing empty shard (if a previous create partially succeeded).
            if (rangeForNewShard < 3)
            {
                shard = CreateOrGetEmptyShard2(shardMap);
            }
            else
            {
                shard = FindEmptyShard3(shardMap, rangeForNewShard % 3);
            }
            

            // Create a mapping to that shard.
            var mappingForNewShard = shardMap.CreatePointMapping(rangeForNewShard, shard);
        ...
        private static Shard CreateOrGetEmptyShard2(ListShardMap<int> shardMap)
        {
            // Get an empty shard if one already exists, otherwise create a new one
            Shard shard = FindEmptyShard2(shardMap);
            if (shard == null)
            {
                // No empty shard exists, so create one
        ...
        private static Shard FindEmptyShard2(ListShardMap<int> shardMap)
        {
            // Get all shards in the shard map
            IEnumerable<Shard> allShards = shardMap.GetShards();
        ...
        private static Shard FindEmptyShard3(ListShardMap<int> shardMap,int selectValue)
        {
            // Get all shards in the shard map
            IEnumerable<Shard> allShards = shardMap.GetShards();
            ...
            // Determine which shards have mappings
            HashSet<Shard> shardsWithMappings = new HashSet<Shard>(allMappings.Where(m => m.Value == selectValue).Select(m => m.Shard));
